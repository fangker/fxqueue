...
class Job extends EventEmitter {
    ...
    async save() {
     ...
     let object= await  new Promise((resolve, reject) => {
            ...
            multi.
                ...
                .hset(hkey, 'attempts', this._attempts || 1)
    ...
    delay(sec) {
        if (typeof sec === 'number') {
            ...
            this._dealy = sec.getTime() - Date.now();
        }
        function isDate() {
    ...
    _toActive(job) {
        return new Promise((resolve, reject) => {
            ...
            multi
                ...
                .hset(hkey, 'attempts', this._attempts = parseInt(this._attempts) + 1)
    ...
    _toRetry() {
        ...
        if (parseInt(this.max_attempts) >= parseInt(this._attempts)) {
            let delay;
            if (typeof this._backoff !== 'object' && this._backoff!== null) {
                delay = this.dealy + Date.now();
            } else if (this._backoff!== null) {
                let { dealy: delay } = this._backoff;
            ...
            multi
                ...
                .hset(getKey('job', this.id), 'delay', delay)
                .zadd(getKey('jobs', 'delay'), this._dealy + Date.now(), this.getZid())
                .zadd(getKey('jobs', this.type, 'delay'), this.priority, this.getZid())
                .zrem(getKey('jobs', this.type, 'delay'), this.getZid())
                .zrem(getKey('jobs:delay'), this.getZid())
                .zrem(getKey('jobs', 'failed'), this.getZid())
                .zrem(getKey('jobs', this.type, 'failed'), this.getZid())
                .exec((err) => {
                    if (err)
                        throw new Error('error')
                    //notification
                    event.emit.bind(this)(this.id, 'retry', this.type, { retry_at: Date.now(), dealy: delay })
                })
        }
    }
    ...
    _toDealy() {
        ...
        multi
            .hset(getKey('job', this.id, false), 'state', this._state)
            .hset(getKey('job', this.id, false), 'delay', this._dealy + Date.now())
            .zadd(getKey('jobs', this._state, false), this._dealy + Date.now(), this.getZid())
            .zadd(getKey('jobs', this.type, this._state, false), this.priority, this.getZid())
            .exec((err) => {
                if (err) {
                    throw new Error('error')

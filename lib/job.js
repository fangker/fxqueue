...
class Job extends EventEmitter {
    ...
    async save() {
        ...
        let object = await new Promise((resolve) => {
            ...
            multi.
                ...
                .hset(hkey, 'ttl', this._ttl||0)
                .hset(hkey, 'delay', this._delay||0)
    ...
    addTypes() {
        this.redis.client.sadd(this.redis.getKey('job', true) + 'types', this.type,(err)=>{
            if(err){
    ...
    addIntoJobs() {
        this.redis.client.zadd(this.redis.getKey('jobs', false), this.priority, this.getZid(),(err)=>{
             super.emit('error',new Error(err))
        })
    }
    getZid() {
    ...
    _inactiveState(job) {
        ...
        return new Promise((resolve, reject) => {
            multi.lpush(getKey(this.type, 'jobs', false), 1)
                ...
                .exec((err) => {
                    if (err) {
                        return super.emit('error', new Error('error'))
                    }
                    resolve(job)
                    //notification
